...
namespace dmGameSystem
{
    ...
    dmGameObject::UpdateResult CompParticleFXOnMessage(const dmGameObject::ComponentOnMessageParams& params)
    {
        ...
        else if (params.m_Message->m_Id == dmGameSystemDDF::SetPositionColliderParticleFX::m_DDFDescriptor->m_NameHash)
        {
            dmGameSystemDDF::SetPositionColliderParticleFX* ddf = (dmGameSystemDDF::SetPositionColliderParticleFX*)params.m_Message->m_Data;
            uint32_t count = world->m_Components.Size();
            for (uint32_t i = 0; i < count; ++i)
            {
                ParticleFXComponent* component = &world->m_Components[i];
